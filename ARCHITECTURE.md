...
| `models/staging/` | 9 views, one per Bronze source (`stg_uniprot`, `stg_string`, `stg_hpa`, `stg_ot_targets`, `stg_ot_diseases`, `stg_ot_associations`, `stg_ot_drugs`, `stg_ot_drug_molecules`, `stg_llm_rewrites`) — each reads via `{{ source('bronze', ...) }}`, declared in `_sources.yml` and backed by a passthrough view (`bronze.<name>`) created by the `on-run-start` hook in `dbt_project.yml`. `source_root` defaults to `r2://atlas-raw` in prod; CI overrides it to `models/fixtures/bronze` (empty-schema stubs) so the full DAG builds without credentials. |
...
| `models/analyses/protein_story_card.sql` | The canonical story-card shape — one row per protein with `LIST(STRUCT(...))` columns for interaction partners, diseases, and drugs. Lives under `analyses/` so dbt compiles and validates it without ever materializing it as a database object; `apps/ui/data.py`'s `STORY_CARD_SQL` is a hand-port of this. |
| `models/tests/` | 11 singular SQL assertions (grain, range, uniqueness, volume/cardinality) beyond dbt's generic `unique`/`not_null`/`relationships`/`accepted_values` tests in `_schema.yml`. |
...
### Bronze sources declared as passthrough views, not inlined `read_parquet()` or copied tables
- **Used**: every staging model reads `{{ source('bronze', 'x') }}`, resolved to a real view (`bronze.uniprot`, `bronze.string`, ...) that the `on-run-start` hook in `dbt_project.yml` (re)creates before every `dbt run`/`build` as `CREATE OR REPLACE VIEW bronze.<name> AS SELECT * FROM read_parquet(<path>)`.
- **Considered**: (1) inlining `read_parquet('{{ var(...) }}/...')` directly in each staging model's `FROM` clause (the original approach); (2) materializing Bronze as real tables in the warehouse.
- **Why**: a bare `read_parquet()` call has no name for dbt to resolve lineage against or introspect columns from, so `dbt docs`'s DAG showed staging models as roots and every source page's column list was empty. A view fixes both — it's a real relation dbt can point `source()` at and introspect via `information_schema`, exactly like it already does for models — while adding zero storage: the view holds no data, just the `read_parquet()` definition, so Bronze still lives only in R2 (see "Bronze as a mandatory landing zone" above). A table was rejected for the same reason that decision rejected writing ingest assets straight into MotherDuck: it would copy Bronze into the warehouse, creating a second copy to keep in sync. Recreating the view on every run is effectively free (no data movement), so the CI fixture stubs and the real R2 root both resolve through the identical mechanism.
...
### `protein_story_card` lives under `analyses/`, never materialized
- **Used**: `models/analyses/protein_story_card.sql` is a dbt analysis — parsed, `ref()`-checked, and compiled on every `dbt compile`/`dbt docs generate`, but never built into the warehouse. Inspect it with `dbt show --select protein_story_card --vars '{accession: ...}'`.
- **Considered**: keeping it under `model-paths` (its original location, `models/queries/`), materialized as a real view on every `dbt run`/`build`.
- **Why**: nothing ever queried that view — `apps/ui/data.py`'s `STORY_CARD_SQL` is a hand-port of the same SQL, kept separate because the Streamlit app has no dbt runtime dependency (see "No API tier" below). A `model-paths` location built a single-row view (hardcoded to the default `accession` var) into the warehouse on every run for zero consumers; `analyses/` keeps the exact same value — a canonical, `ref()`-validated spec the hand-port is checked against — without the unused database object.
