...
`duckdb==1.5.2` pinned to MotherDuck's extension cap — re-evaluate when MotherDuck upgrades. Staging initially read Bronze via inline `read_parquet('{{ var("source_root") }}/...')` rather than dbt `source()` — simpler and configurable (`source_root` defaults to `r2://atlas-raw` in prod, overridden to local fixtures in CI). **Superseded in the Part 9 deploy branch** — staging now reads `{{ source('bronze', ...) }}` backed by passthrough views (see "Bronze sources via source() passthrough views" below). `maxClinicalStage` in OT v26.03 is a string (`"PHASE_3"`) — mapped to SMALLINT in `dim_drug` via explicit CASE with no ELSE (unknown stages become NULL and fail the `accepted_values` test loudly).
...
## Bronze sources via source() passthrough views

Reversed the Part 3 inline-`read_parquet` decision. Each staging model now reads `{{ source('bronze', <name>) }}`, declared in `models/staging/_sources.yml`. Before any model runs, the `on-run-start` hook in `dbt_project.yml` creates one `bronze.<name>` view per source — `CREATE OR REPLACE VIEW bronze.<name> AS SELECT * FROM read_parquet(<path>)` — via the `bronze_view_sql` macro in `models/macros/bronze_views.sql`. Why: a bare `read_parquet()` has no relation for dbt to resolve lineage against or introspect columns from, so `dbt docs` rendered staging models as DAG roots with empty source column lists. A view is a real, introspectable relation (`source()` resolves to it; `information_schema` sees its columns) yet copies no data — Bronze still lives only in R2. Rejected materializing Bronze as tables: that would duplicate R2 into the warehouse (the same reason ingest assets don't write straight to MotherDuck). Recreating the views each run is free (no data movement), so CI fixtures and the real R2 root resolve through the identical mechanism. Full rationale in `ARCHITECTURE.md`, "Bronze sources declared as passthrough views."

## Column docs + public dbt docs site

Documented every staging and mart column in the `_schema.yml` files (previously only tested/join columns had descriptions); shared definitions (`uniprot_accession`, `efo_id`, `chembl_id`, `ensembl_gene_id`) live in `{% docs %}` blocks in `models/marts/_docs.md`, with an `__overview__` block for the docs homepage. `.github/workflows/dbt-docs.yml` builds against a new file-based `docs` target (not `ci`'s `:memory:` — `dbt build` and `dbt docs generate` are separate CLI invocations and must share the same materialized relations, else the catalog introspects an empty DB and every column type is missing) loaded from the zero-row Bronze fixtures, then publishes the static site to GitHub Pages on push to `main`. No `MOTHERDUCK_TOKEN` in that workflow — its output is a public webpage, so credential exposure is avoided at the cost of row counts reading 0 (names/types/lineage are accurate). Full rationale in `ARCHITECTURE.md`, "Public dbt docs site."

## protein_story_card moved to analyses/

Moved `protein_story_card.sql` from `models/queries/` (a `model-paths` dir, so it materialized a dead single-row view — hardcoded to the default `accession` var — into the warehouse on every run for zero consumers) to `models/analyses/`. dbt now parses, `ref()`-checks, and compiles it on every run but never materializes it; inspect with `dbt show --select protein_story_card --vars '{accession: ...}'`. `apps/ui/data.py`'s `STORY_CARD_SQL` stays the hand-port, and the analysis is the canonical, `ref()`-validated spec it's checked against. `model-paths` in `dbt_project.yml` narrowed to `["staging", "marts"]`. Full rationale in `ARCHITECTURE.md`, "`protein_story_card` lives under `analyses/`, never materialized."
